...
# Time-frequency analysis parameters matching test_tf.jl
time_of_interest = -0.5:0.001:1.0
frequency_of_interest = 1:0.5:80  # Match test_tf.jl
# Run time-frequency analysis
# width=[3, 10] means 3 cycles at lowest freq, 10 at highest (logarithmic scaling)
tf_data = eegfun.tf_analysis(epochs, frequency_of_interest, time_of_interest; method=:wavelet, width=(3, 10))
# Plot with baseline correction matching test_tf.jl
fig, ax = eegfun.plot_time_frequency(tf_data, :Channel1; 
                                      title="Figure 13.14 - Time-Frequency Analysis", 
                                      baseline_window=(-0.5, -0.2), 
                                      baseline_method=:db,
                                      ylogscale=true, colorrange=(-3, 3), colormap=:jet)
