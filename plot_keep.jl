...
function plot_databrowser(dat::ContinuousData, channel_labels::Vector{Symbol}, ica::Union{InfoIca,Nothing} = nothing)
    ...
    function build_grid_components(dat, toggles, menu_ica=nothing)
        ...
        if dat.analysis_info.hp_filter == 0.0 && isnothing(menu_ica)
            push!(grid_components, 
                hcat(slider_hp_filter,
                    Label(fig, @lift("HP-Filter: $($(slider_hp_filter.value)) Hz"), 
                    fontsize = 22, halign = :left))
            )
        end
        if dat.analysis_info.lp_filter == 0.0
            push!(grid_components,
                hcat(slider_lp_filter,
                    Label(fig, @lift("LP-Filter: $($(slider_lp_filter.value)) Hz"), 
                    fontsize = 22, halign = :left))
            )
        end
