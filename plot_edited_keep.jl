...
function butterfly_plot!(active::Bool, state::DataBrowserState)
    if active
        ...
        state.offset[] =
            LinRange((state.yrange.val[end] * 0.9), state.yrange.val[1] * 0.9, length(state.channel_labels))
    end
...
function plot_databrowser(dat::ContinuousData, channel_labels::Vector{Symbol}, ica::Union{InfoIca,Nothing} = nothing)
    ...
    for t = 1:length(toggles[:, 1])
        ...
        elseif toggles[t, 2].text.val == "Butterfly Plot"
            on(toggles[t, 1].active) do _
    ...
    if ("is_vEOG" in names(state.data_obs[]) && "is_hEOG" in names(state.data_obs[]))
    ...
    on(labels_menu[1].selection) do s
        ...
        if s == "All"
            state.channel_labels = state.channel_labels_original
        elseif s == "Left"
            state.channel_labels = state.channel_labels_original[findall(
                occursin.(r"\d*[13579]$", String.(state.channel_labels_original)),
            )]
        elseif s == "Right"
            state.channel_labels = state.channel_labels_original[findall(
                occursin.(r"\d*[24680]$", String.(state.channel_labels_original)),
            )]
        elseif s == "Central"
            state.channel_labels =
    ...
    reference_menu = hcat(
        Menu(
            ...
            default = String(state.data_original.analysis_info.reference),
            direction = :down,
            fontsize = 18,
            width = 200,  # Match width
        ),
        ...
        on(menu_ica[1].selection) do s
            ...
            if !isnothing(components_to_remove)
                if !isnothing(components_removed)
                    ...
                    state.data_obs[] =
                        restore_original_data(state.data_obs[], ica_result, [components_removed], removed_activations)
                end
                state.data_obs[], removed_activations =
                    remove_ica_components(state.data_obs[], ica, [components_to_remove])
    ...
    on(extra_menu[1].selection) do s
        ...
        if eltype(data_obs[][!, s]) == Bool
            ...
            extra_label = text!(
                ax,
                @lift($(state.data_obs).time[$(state.xrange)[1]]),  # Use first visible timepoint
                @lift(begin
                    df = $(state.data_obs)
                    current_offset = $(state.offset)[end] + mean(diff($(state.offset)))
                    df[!, s][$(state.xrange)[1]] .+ current_offset  # Use value at first visible timepoint
                end),
                text = String(s),
                align = (:left, :center),
                fontsize = 18,
    ...
    function build_grid_components(dat, toggles, menu_ica = nothing)
        ...
        if dat.analysis_info.hp_filter == 0.0 && isnothing(menu_ica)
            push!(
                grid_components,
                hcat(
                    slider_hp_filter,
        ...
        if !isnothing(menu_ica)
            push!(common_controls, menu_ica)
        end

        # Add extra menu
        push!(common_controls, extra_menu)
        append!(grid_components, common_controls)

        return grid_components
    end

    # GUI Control Panel
    grid_components = build_grid_components(dat, toggles, menu_ica)
