...
function process_eeg_to_grandaverage(raw_files::Vector{String}, data_dir::String, run_ica::Bool)
    ...
    for (i, file) in enumerate(raw_files)
        ...
        if run_ica
            ...
            if i == 1
                fig_ica = EegFun.plot_topography(ica_result, component_selection=EegFun.components(1:30), colorbar_plot=false, label_plot=false, point_plot=false)
                resize!(fig_ica.fig, 1200, 1000)
                save(joinpath(data_dir, "benchmark_julia_ica.pdf"), fig_ica.fig)
            end

            # 4. Remove ICA component
            EegFun.remove_ica_components!(
                dat,
                ica_result,
    ...
    # 7. Grand Average
    grand_avg = EegFun.grand_average(all_subject_erps)
...
Base.invokelatest(process_eeg_to_grandaverage, test_files, data_dir, run_ica_flag) # Warm-up to trigger JIT compilation
