...
function plot_comparison()
    ...
    ax1 = Axis(fig_combined[1, 1], xlabel="Time (s)", ylabel="Amplitude (µV)", title="Combined Cross-Pipeline Comparison (PO7/PO8)", xgridvisible=false, ygridvisible=false)
    ...
    ax4 = Axis(fig_indiv[3, 1], xlabel="Time (s)", ylabel="Amplitude (µV)", title="MATLAB (EEGLAB)", xgridvisible=false, ygridvisible=false)
    ...
    if !ismissing(python_data)
        ...
        lines!(ax1, python_data[:, 1], python_data[:, 3], color=c_python, linestyle=:dash, linewidth=2, label="$lbl_python (Invalid)")
        # Python only plot
    ...
    if !ismissing(matlab_data)
        ...
        lines!(ax1, matlab_data[:, 1], matlab_data[:, 3], color=c_matlab, linestyle=:dash, linewidth=2, label="$lbl_matlab (Invalid)")
        # MATLAB only plot
