...
function _peak_vec(trials::Int, samples::Int, sample_rate::Int, 
                  peak_freq::Float64, peak_amp::Float64, peak_latency::Float64, 
                  jitter_amp::Float64, jitter_latency::Float64)
...
function _noise_vec(trials::Int, samples::Int, sample_rate::Int, noise_amp::Float64)
    ...
    meanpower = [0.0015; 0.0008; 0.0006; 0.0005; fill(0.0004, 3); fill(0.0003, 2); 
                fill(0.0004, 2); fill(0.0003, 3); fill(0.0002, 42); fill(0.0001, 88)]
...
function _simulate_my_erp(trials::Int, comps::Matrix{Float64}; 
                        samp_freq::Int = 1000, 
    ...
    for comp in 1:n_components
        my_comp[:, :, comp] = _peak_vec(trials, length(time), samp_freq,
...
function simulate_erp()
    ...
    ax = Axis(gl[1, 1:2], 
              xlabel="Time (ms)", 
              ylabel="Amplitude (μV)",
              title="Interactive ERP Simulation Demo")
    ...
    function update_plot(new_signals=nothing, new_erp=nothing)
        ...
        if any(c -> c.active[], components)
            ...
            for i in 1:size(current_signals, 1)
