...
fig, ax = plot_epochs(dat, channels([:Fp1, :Fp2, :F3, :F4]))

# Plot single channel
fig, ax = plot_epochs(dat, channels(:Fp1))
```

## Channel Filtering
```julia
# Exclude reference channels
fig, ax = plot_epochs(dat, channels_not([:M1, :M2]))

# Plot frontal channels only
fig, ax = plot_epochs(dat, channels(1:10))

# Plot channels starting with "F" (frontal)
fig, ax = plot_epochs(dat, x -> startswith.(string.(x), "F"))

# Mix of layout and additional channels
fig, ax = plot_epochs(dat, channels([:Fp1, :Fp2, :vEOG, :hEOG]))
```
...
# Custom grid layout
fig, ax = plot_epochs(dat, dims = [4, 4])  # 4x4 grid

# Hide decorations for clean look
fig, ax = plot_epochs(dat, hidedecorations = true)

# Reverse Y-axis (positive up)
fig, ax = plot_epochs(dat, yreversed = true)
```
...
fig, ax = plot_epochs(dat, channels_not([:M1, :M2, :vEOG, :hEOG]))

# Frontal and parietal channels
fig, ax = plot_epochs(dat, x -> 
    startswith.(string.(x), "F") .| startswith.(string.(x), "P")
)

# Channels with specific indices
fig, ax = plot_epochs(dat, channels([1, 5, 10, 15, 20]))
```
