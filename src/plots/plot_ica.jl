...
"""
    plot_ica_component_spectrum(ica_result::InfoIca, dat::ContinuousData, component_indices::Union{Int,Vector{Int},UnitRange}; kwargs...)

Plot the power spectrum of specified ICA components.

# Arguments
- `ica_result::InfoIca`: The ICA result object.
- `dat::ContinuousData`: The continuous data object.
- `component_indices::Union{Int,Vector{Int},UnitRange}`: Component indices to plot.
...
fig, ax = plot_ica_component_spectrum(ica_result, dat, 1)

# Plot multiple components
fig, ax = plot_ica_component_spectrum(ica_result, dat, [1, 3, 5])

# Plot range of components
fig, ax = plot_ica_component_spectrum(ica_result, dat, 1:10)
```
"""
function plot_ica_component_spectrum(ica_result::InfoIca, dat::ContinuousData, component_indices::Union{Int,Vector{Int},UnitRange}; kwargs...)
    # Convert single index to vector
    if component_indices isa Int
        component_indices = [component_indices]
    end
    
    # Validate component indices
    n_components = length(ica_result.ica_label)
    for comp_idx in component_indices
        if comp_idx < 1 || comp_idx > n_components
            throw(ArgumentError("Component index $comp_idx is out of range [1, $n_components]"))
        end
    end
    ...
    for (i, comp_idx) in enumerate(component_indices)
        ax = Axis(fig[i, 1], title = "Component $comp_idx")
        
        # Calculate power spectrum
        fs = dat.sample_rate
        freqs, psd = welch(components[comp_idx, :], fs)
...
"""
    plot_ica_component_spectrum(ica_result::InfoIca, dat::ContinuousData, component_selection::Function; kwargs...)

Plot the power spectrum of ICA components using a component selection predicate.

# Arguments
- `ica_result::InfoIca`: The ICA result object.
- `dat::ContinuousData`: The continuous data object.
- `component_selection::Function`: Function that returns boolean vector for component filtering.
- `kwargs`: Additional keyword arguments for customization.
...
fig, ax = plot_ica_component_spectrum(ica_result, dat, components([1, 3, 5]))

# Plot components 1-10
fig, ax = plot_ica_component_spectrum(ica_result, dat, components(1:10))

# Plot all components except 1 and 2
fig, ax = plot_ica_component_spectrum(ica_result, dat, components_not([1, 2]))

# Plot component 1 only
fig, ax = plot_ica_component_spectrum(ica_result, dat, components(1))
```
"""
function plot_ica_component_spectrum(ica_result::InfoIca, dat::ContinuousData, component_selection::Function; kwargs...)
    selected_components = get_selected_components(ica_result, component_selection)
    return plot_ica_component_spectrum(ica_result, dat, selected_components; kwargs...)
end
