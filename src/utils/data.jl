...
"""
    subset_view(dat::SingleDataFrameEeg; 
               channel_selection::Function = channels(), 
               sample_selection::Function = samples(), 
               include_extra_channels::Bool = false) -> SingleDataFrameEegView
...
# Returns
- SingleDataFrameEegView object that references the original data (no copying at all)
"""
function subset_view(dat::SingleDataFrameEeg; 
    ...
    return SingleDataFrameEegView(dat, selected_samples, selected_channels, dat.layout)
end

# Add subset_view for MultiDataFrameEeg
"""
    subset_view(dat::MultiDataFrameEeg; 
               channel_selection::Function = channels(), 
               sample_selection::Function = samples(), 
               include_extra_channels::Bool = false) -> MultiDataFrameEegView

Create a true view of a subset of MultiDataFrameEeg without any copying.

# Arguments
- `dat`: MultiDataFrameEeg object to subset (EpochData)
- `channel_selection`: Function that returns channel labels to include (default: all channels)
- `sample_selection`: Function that returns sample mask to include (default: all samples)
- `include_extra_channels`: Whether to include additional channels (default: false)

# Returns
- MultiDataFrameEegView object that references the original data (no copying at all)
"""
function subset_view(dat::MultiDataFrameEeg; 
                   channel_selection::Function = channels(), 
                   sample_selection::Function = samples(), 
                   include_extra_channels::Bool = false)
    ...
    # Create view wrapper that references the original data (no copying)
    return MultiDataFrameEegView(dat, selected_samples, selected_channels, dat.layout)
end

# === VIEW TYPE METHODS ===
# Make view types behave like other EegData objects

# Sample count
n_samples(view::SingleDataFrameEegView)::Int = length(view.selected_samples)
n_samples(view::MultiDataFrameEegView)::Int = length(view.selected_samples)

# Channel count
n_channels(view::SingleDataFrameEegView)::Int = length(view.selected_channels)
n_channels(view::MultiDataFrameEegView)::Int = length(view.selected_channels)

# Epoch count
n_epochs(view::SingleDataFrameEegView)::Int = 1
n_epochs(view::MultiDataFrameEegView)::Int = length(view.original_data.data)

# Duration
duration(view::SingleDataFrameEegView) = last(view.original_data.data.time[view.selected_samples]) - first(view.original_data.data.time[view.selected_samples])
duration(view::MultiDataFrameEegView) = last(view.original_data.data[1].time[view.selected_samples]) - first(view.original_data.data[1].time[view.selected_samples])

# Channel labels
channel_labels(view::SingleDataFrameEegView)::Vector{Symbol} = view.selected_channels
channel_labels(view::MultiDataFrameEegView)::Vector{Symbol} = view.selected_channels

# All labels
all_labels(view::SingleDataFrameEegView)::Vector{Symbol} = view.selected_channels
all_labels(view::MultiDataFrameEegView)::Vector{Symbol} = view.selected_channels

# All data
all_data(view::SingleDataFrameEegView)::DataFrame = view.data
all_data(view::MultiDataFrameEegView)::Vector{DataFrame} = view.data

# Meta data
meta_data(view::SingleDataFrameEegView)::DataFrame = view.original_data.data[view.selected_samples, get_cols_by_group(view.original_data, :metadata)]
meta_data(view::MultiDataFrameEegView)::Vector{DataFrame} = [df[view.selected_samples, get_cols_by_group(view.original_data, :metadata)] for df in view.original_data.data]

# Channel data
channel_data(view::SingleDataFrameEegView)::DataFrame = view.data
channel_data(view::MultiDataFrameEegView)::Vector{DataFrame} = view.data

# Extra data
extra_data(view::SingleDataFrameEegView)::DataFrame = view.original_data.data[view.selected_samples, get_cols_by_group(view.original_data, :extra)]
extra_data(view::MultiDataFrameEegView)::Vector{DataFrame} = [df[view.selected_samples, get_cols_by_group(view.original_data, :extra)] for df in view.original_data.data]
