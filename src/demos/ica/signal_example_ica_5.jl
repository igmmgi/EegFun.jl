...
function signal_example_ica_5()
    ...
    for _ in 1:(N÷400)
        append!(s1, zeros(300))
        append!(s1, [exp(-abs(i)/10.0) for i in -50:49])
    end
    # Source 2: oscillatory bursts (like alpha)
    for _ in 1:(N÷500)
        append!(s2, zeros(300))
        append!(s2, [1.5 * sin(2π * i / 20) * exp(-(i/40)^2) for i in -100:99])
    end
    ...
    # ── Build Layout ──────────────────────────────────────────────────────────
    Label(fig[0, 1:2], "Inside the Black Box: How ICA Finds Sources",
          fontsize = @lift($title_sz + 4), font = :bold, halign = :center)

    # Panel 1: Scatter and Projection Vector
    ax_scatter = Axis(fig[1, 1]; title = "The Search Space\n(Data is 'Sphered' to be mathematically round, so ICA only needs to search rotations)", 
    ...
    # Panel 2: Histogram
    ax_hist = Axis(fig[1, 2]; title = "Projected 1D Data Distribution",
    ...
    x_range = -5:0.1:5
    lines!(ax_hist, x_range, x -> exp(-x^2/2) / sqrt(2π); color = (:red, 0.5), linestyle = :dash, linewidth = 2, label="Gaussian (Target=0)")
    ...
    ax_land = Axis(fig[3, 1:2]; title = "Objective Function Landscape (Kurtosis vs. Angle)", 
                   xlabel = "Vector Angle (Degrees)", ylabel = "Non-Gaussianity (Kurtosis)",
                   titlesize = title_sz, xlabelsize = lbl_sz, ylabelsize = lbl_sz)
