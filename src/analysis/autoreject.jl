        ...
        for (c_idx, cons) in enumerate(params.consensus)
            for (t_idx, thresh) in enumerate(params.thresh_range)
                ...
                if params.thresh_method == :mad
                    channel_thresh = median(p2p, dims=2) .+ 
                                   thresh .* mads(p2p, dims=2)
                else # :std
                    channel_thresh = mean(p2p, dims=2) .+ 
                                   thresh .* std(p2p, dims=2)
                end
                # Find violations
...
function repair_epochs!(epochs::EpochData, bad_channels::Dict, neighbours::Dict)
    for (epoch_idx, bad_chans) in bad_channels
        ...
        for bad_chan in bad_chans
            ...
            weights = [1/neighbours[Symbol(bad_chan)].distances[findfirst(==(n), neighbor_labels)] 
...
function plot_interpolation_comparison(epochs::EpochData, bad_channels::Dict, epoch_idx::Int)
    ...
    for chan in epochs.layout.label
        lines!(ax1, epoch_data.time, epoch_data[!, chan],
               color=:lightgrey,
               linewidth=1,
               alpha=0.5,
               inspectable=true)
    ...
    for chan in epochs.layout.label
        ...
        lines!(ax2, epoch_data.time, epoch_data[!, chan], 
               color=color,
               linewidth=1,
               alpha=0.5,
               inspectable=true)
    ...
    for chan in epochs.layout.label
        color = chan in bad_channels[epoch_idx] ? :red : :lightgrey
        lines!(ax3, epochs_copy.data[epoch_idx].time, epochs_copy.data[epoch_idx][!, chan],
               color=color,
               linewidth=1,
               alpha=0.5,
               inspectable=true)
        ...
        if event.button == Mouse.left && event.action == Mouse.press
            ...
            if plt !== nothing && plt[1] isa Lines
                ...
                for (i, chan) in enumerate(epochs.layout.label)
                    if plt[1] == ax1.scene.plots[i] || 
                       plt[1] == ax2.scene.plots[i] || 
                       plt[1] == ax3.scene.plots[i]
