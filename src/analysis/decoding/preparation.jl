...
"""
    prepare_decoding(epochs::Vector{TimeFreqEpochData}; condition_selection, channel_selection, interval_selection)

Prepare TimeFreqEpochData for multivariate pattern analysis (MVPA/decoding).

Organizes TimeFreqEpochData into a structure suitable for TF decoding analysis by grouping
epochs by participant and selecting specified conditions for classification.

# Arguments
- `epochs::Vector{TimeFreqEpochData}`: TF epoch data containing multiple conditions/participants
...
# Returns
- `Vector{Vector{TimeFreqEpochData}}`: Vector of participant data, where each element is a vector of TimeFreqEpochData for that participant's conditions

# Examples
```julia
# Load TF epoch data (computed with return_trials=true)
...
function prepare_decoding(
    ...
    # Apply condition selection to the sorted condition numbers
    all_cond_nums = collect(keys(epochs_by_condition))
    selected_mask = condition_selection(1:length(all_cond_nums))
    selected_cond_nums = all_cond_nums[selected_mask]
    ...
    for (cond_idx, condition_epochs) in enumerate(selected_conditions)
        _have_same_structure(condition_epochs) ||
            @minimal_error("Condition $(selected_cond_nums[cond_idx]): TF epochs have inconsistent structure")
    end
    ...
    participant_epochs = Vector{Vector{TimeFreqEpochData}}()
    sizehint!(participant_epochs, length(unique_participants))

    for participant_id in unique_participants
        participant_data = Vector{TimeFreqEpochData}()
        sizehint!(participant_data, length(selected_conditions))
...
"""
    prepare_decoding(file_pattern::String, ::Type{TimeFreqEpochData}; input_dir, participant_selection, condition_selection, channel_selection, interval_selection)

Prepare TimeFreqEpochData for decoding from JLD2 files (convenience wrapper).

# Arguments
- `file_pattern::String`: Pattern to match JLD2 files (e.g., "tf_epochs")
- `::Type{TimeFreqEpochData}`: Data type to load
...
# Returns
- `Vector{Vector{TimeFreqEpochData}}`: Vector of participant data

# Examples
```julia
participant_tf = prepare_decoding(
    "tf_epochs", TimeFreqEpochData,
    input_dir = "/path/to/data",
    condition_selection = conditions([1, 2]))
```
"""
