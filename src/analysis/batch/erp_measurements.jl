...
function _validate_analysis_window(window::Tuple{Real, Real})
    window[1] >= window[2] && 
        return "Analysis window start must be before end. Got: $window"
    return nothing
end
...
"""
function _compute_measurement(chan_data::AbstractVector, time_col::AbstractVector, 
                            time_idx::AbstractVector, analysis_type::String)
...
"""
function _process_dataframe_measurements(df::DataFrame, selected_channels::Vector{Symbol},
                                       analysis_window::Tuple{Real, Real}, 
                                       analysis_type::String,
                                       participant::Int, condition::Int,
                                       has_condition_name::Bool, has_epoch::Bool)
...
"""
function _process_measurements_file(filepath::String, 
                                   analysis_window::Tuple{Real, Real},
                                   analysis_type::String,
    ...
    for (cond_idx, data) in enumerate(data_var)
        ...
        for df in dfs_to_process
            row_data = _process_dataframe_measurements(df, selected_channels, 
                                                     analysis_window, analysis_type,
                                                     participant, condition,
                                                     has_condition_name, has_epoch)
...
function erp_measurements(file_pattern::String, 
    ...
    try
        ...
        for (file_idx, file) in enumerate(files)
            ...
            try
                file_results = _process_measurements_file(input_path, analysis_window, 
                                                        analysis_type, baseline_interval,
                                                        conditions, channel_selection)
