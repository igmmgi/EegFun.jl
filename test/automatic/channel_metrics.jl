...
@testset "correlation" begin
    ...
    @testset "correlation_matrix" begin
        ...
        dat = EegFun.create_test_continuous_data(n = 1000, n_channels = 100)
    ...
    @testset "correlation_matrix_dual_selection" begin
        dat = EegFun.create_test_continuous_data(n = 1000, n_channels = 10)
    ...
    @testset "correlation_matrix_eog" begin
        ...
        # Test with sample_selection (create a predicate function)
        sample_pred = x -> [i <= 500 for i = 1:nrow(x)]
        cm = EegFun.correlation_matrix_eog(dat, eog_cfg, sample_selection = sample_pred)
        @test cm isa DataFrame

        # Test with channel_selection
        cm = EegFun.correlation_matrix_eog(dat, eog_cfg, channel_selection = EegFun.channels([:Ch1, :Ch2, :Ch3]))
        @test size(cm, 1) == 3

        # Test with include_extra
        cm = EegFun.correlation_matrix_eog(dat, eog_cfg, include_extra = true)
        @test cm isa DataFrame
    end
    ...
    @testset "identify_bad_channels" begin
        ...
        summary_good = DataFrame(channel = [:Ch1, :Ch2], zvar = [1.0, 1.5], variance = [0.1, 0.2])
        joint_prob_good = DataFrame(channel = [:Ch1, :Ch2], jp = [0.1, 0.2], rejection = [false, false])
    ...
    @testset "partition_channels_by_eog_correlation" begin
        ...
        non_eog, eog_related = EegFun.partition_channels_by_eog_correlation(bad_channels, eog_corr_df)
        ...
        non_eog, eog_related = EegFun.partition_channels_by_eog_correlation(bad_channels, eog_corr_df, threshold = 0.5)
        @test :Ch1 in non_eog
        ...
        eog_corr_df_z = DataFrame(row = [:Ch1, :Ch2, :Ch3], z_hEOG = [0.1, 0.5, 0.2], z_vEOG = [0.15, 0.2, 0.6])
        bad_channels_z = [:Ch1, :Ch2, :Ch3]
        non_eog, eog_related = EegFun.partition_channels_by_eog_correlation(bad_channels_z, eog_corr_df_z, use_z = true)
        @test :Ch1 in non_eog
        @test :Ch2 in eog_related
        @test :Ch3 in eog_related

        # Test with empty bad_channels
        non_eog, eog_related = EegFun.partition_channels_by_eog_correlation(Symbol[], eog_corr_df)
        ...
        non_eog, eog_related = EegFun.partition_channels_by_eog_correlation(bad_channels_missing, eog_corr_df)
    ...
    @testset "check_channel_neighbors" begin
        ...
        layout_df = DataFrame(label = channel_labels, inc = zeros(5), azi = zeros(5))
