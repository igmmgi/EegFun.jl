...
function create_participant_condition_epochs(
    ...
    sample_rate::Int = 200,  # 200 points over 1 second (from -0.2 to 0.8s)
    channels::Vector{Symbol} = [:Fz, :Cz, :Pz, :POz],
)
    # Create time vector and layout based on parameters
    ...
    for epoch = 1:n_epochs
        ...
        for (ch_idx, ch) in enumerate(channels)
            ...
            if condition_id == 1 # Condition 1: Positive ERP component
                signal =
                    signal_strength * exp.(-((time .- 0.2) .^ 2) / (2 * 0.05^2)) .+
                    0.3 * signal_strength * exp.(-((time .- 0.4) .^ 2) / (2 * 0.08^2))
            else # Condition 2: Negative ERP component
                signal =
                    -signal_strength * exp.(-((time .- 0.2) .^ 2) / (2 * 0.05^2)) .+
...
for p = 1:n_participants
    ...
    cond1 = create_participant_condition_epochs(p, 1, "Cond1", n_epochs_per_condition, 0.1)
    # Participant p, Condition 2
    cond2 = create_participant_condition_epochs(p, 2, "Cond2", n_epochs_per_condition, 0.1)
...
all_decoded = [EegFun.decode_libsvm(
    epochs;
...
fig, ax = EegFun.plot_decoding(grand_avg_decoded, title = "Test Decoding")
...
fig_stats_synthetic = EegFun.plot_decoding(grand_avg_decoded, stats_synthetic, title = "Synthetic Data with Significance")
