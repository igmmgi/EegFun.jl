...
function plot_databrowser(dat::EpochData, channel_labels::Vector{<:AbstractString})
  ...
  function draw(; plot_labels=true)
    ...
    for col in channel_labels
      # original data
      channel_data_original[col] = lines!(ax, data[trial.val][!, :time], data[trial.val][!, col], color=@lift(abs.(dat.data[trial.val][!, col]) .>= $crit_val), colormap=[:darkgrey, :darkgrey, :red], linewidth=linewidth_orig, alpha=alpha_orig)
      if plot_labels
        channel_data_labels[col] = text!(ax, @lift(data[trial.val][$xrange, :time][1]), @lift(data[trial.val][$xrange, col][1]), text=col, align=(:left, :center), fontsize=20)
      end
      # also show filtered data
      if plot_filtered_data
