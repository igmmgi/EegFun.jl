...
"""
function infomax_ica(dat::Matrix{Float64}; params::IcaPrms = IcaPrms(), n_components::Union{Nothing,Int} = nothing)

    dat_ica = copy(dat)
    dat_ica = scale_data(dat_ica)
    ...
    return InfoIca(weights, sphere, mixing, unmixing, labels)
end
...
function scale_data(data::Matrix{Float64})
    ...
    # Normalize the data (divide by standard deviation)
    # data_scaled = data_centered ./ std(data_centered, dims=2)
    return data_scaled
end
