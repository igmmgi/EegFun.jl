...
![Paradigm figure showing trial sequence: fixation cross (800 ms), then face or body part stimulus (200 ms), then blank ITI (1000 ms)](/demos/experiments/n170/n170_paradigm.png)
...
![Biosemi 72-channel layout](/demos/experiments/n170/biosemi72.png)
...
![Databrowser showing vEOG channel with detected blink onsets highlighted](/demos/experiments/n170/veog.png)
...
Exclude EOG blink regions when assessing bad channels — mark the blink windows first, then recompute metrics with `samples_not` (`sample_selection` goes on the metric functions, not on `identify_bad_channels`):

```julia
EegFun.mark_epoch_intervals!(dat, [:is_vEOG, :is_hEOG], [-0.2, 0.4]; channel_out = :eog_window)

cs_no_eog = EegFun.channel_summary(dat,
    sample_selection = EegFun.samples_not(:eog_window)
)
EegFun.viewer(cs_no_eog)
jp_no_eog = EegFun.channel_joint_probability(dat,
    sample_selection = EegFun.samples_not(:eog_window)
)
bad_channels_no_eog = EegFun.identify_bad_channels(cs_no_eog, jp_no_eog)
```

Browse again to see the EOG windows alongside extreme-value highlights:

```julia
# Browse — EOG windows now visible as shaded regions alongside extreme-value highlights
EegFun.plot_databrowser(dat)
```
...
ica_result = EegFun.run_ica(dat_ica, channel_selection = EegFun.channels_not(bad_channels))
...
```julia
# View data with ICA components available for selection
...
EegFun.plot_erp("grand_average_erps_good.jld2", baseline_interval = EegFun.times(-0.2, 0), 
    channel_selection = EegFun.channels([:P7, :P8, :PO7, :PO8]), layout = :grid, ylim = (-5, 15))
```

![Grand average ERP waveforms at the N170 ROI (P7, P8, PO7, PO8)](/demos/experiments/n170/n170_erp.png)
...
![Topographic map at the N170 time window (~170 ms)](/demos/experiments/n170/n170_topo.png)
...
```julia
EegFun.condition_difference("grand_average_erps_good.jld2", [[2, 1]])
```
...
![Difference wave (Face − Body) at the N170 ROI](/demos/experiments/n170/n170_erp_diff.png)

Topography of the difference wave at the N170 peak:

```julia
EegFun.plot_topography("grand_average_erps_good.jld2", interval_selection = EegFun.times(0.17), ylim = (-3, 3))
```

![Difference wave topography at ~170 ms](/demos/experiments/n170/n170_topo_diff.png)
...
EegFun.plot_erp_measurement_gui("example1_erps_good.jld2")
```

![ERP Measurement GUI](/demos/experiments/n170/n170_measurement_erp.png)
