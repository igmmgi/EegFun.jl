...
"""
    filter_data!(dat::DataFrame, columns, filter_type, filter_method, filter_freq, sample_rate; 
                order=3, transition_width=0.25, print_filter_characteristics=true, plot_filter_response=false)
...
- `transition_width`: Relative width of transition band (default: 0.25)
...
function filter_data!(
    ...
    if !(isa(filter_freq, Real) && filter_freq > 0 && filter_freq < sample_rate / 2)
...
function filter_data(
    ...
    filter_data!(
        ...
        twopass = twopass,
        print_filter_characteristics = print_filter_characteristics,
        plot_filter_response = plot_filter_response,
    )
    return dat_out
end
...
function filter_data!(
    ...
    twopass::Bool = true,
    print_filter_characteristics = true,
...
"""
    plot_filter_response(filter, sample_rate::Real, filter_freq::Union{Real,Tuple}, transition_band::Real; 
                        ylimit::Tuple=(-100, 5))
